...
export async function POST(req: NextRequest, { params }: { params: { code: string } }) {
  try {
    ...
    const parseResult = castVoteSchema.safeParse(json);
    if (!parseResult.success) {
      return jsonError('BAD_REQUEST', parseResult.error.errors[0]?.message);
    }

    const { targetPlayerId } = parseResult.data;
    ...
    const { snapshot, actingPlayer } = await store.mutateRoom(code, tokenHash, (snap, player) => {
      ...
      return castVote(snap.room, snap.players, snap.messages, snap.votes, player.id, targetPlayerId);
    });
